...
class PolicyConfig:
    ...
    def from_env(cls) -> "PolicyConfig":
        raw = os.getenv("OWNER_DISCORD_ID", "").strip()
        if not raw:
            raise RuntimeError("OWNER_DISCORD_ID is required.")
        try:
            owner_id = int(raw)
        except ValueError as exc:
            raise RuntimeError("OWNER_DISCORD_ID must be an integer Discord ID.") from exc
        return cls(owner_discord_id=owner_id)
...
class OwnerGatedGuard:
    ...
    def apply_owner_approval(
        ...
        duration_minutes: int,
        now: datetime | None = None,
    ) -> tuple[bool, Grant | None]:
        """
        消息级授权入口：
        - 仅 owner 的授权消息可生效
        - 其他人的“授权”尝试全部忽略（返回 False, None）
        """
