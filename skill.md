...
- Bot-to-bot tutoring must strictly stop at `expires_at`; no grace period, no extra turn.
...
- Before every outbound message, re-check `now < expires_at`; if false, terminate immediately.
...
- Authorization time reached (`now >= expires_at`) for either side.
...
Time guard (strict):
- In any bot-to-bot session, if `now >= expires_at`, force `CLOSED` immediately.
- Send timeout close message, @requester_owner, and do not send any further bot-to-bot content.
