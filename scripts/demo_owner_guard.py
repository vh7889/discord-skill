...
def main() -> None:
    ...
    # 非主人尝试授权：必须失败（不生效）
    ok, grant = guard.apply_owner_approval(
        sender_id=user_id, requester_id=user_id, duration_minutes=1, now=now
    )
    print("non-owner approval:", ok, grant)

    # 主人授权 1 分钟：成功
    ok, grant = guard.apply_owner_approval(
        sender_id=owner_id, requester_id=user_id, duration_minutes=1, now=now
    )
    print("owner approval:", ok, grant)
